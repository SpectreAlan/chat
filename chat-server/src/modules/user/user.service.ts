...
export class UserService {
    ...
    async updateUserName(user: User) {
        try {
            const oldUser = await this.userRepository.findOne({userId: user.userId});
            if (oldUser && nameVerify(user.username)) {
                const isHave = await this.userRepository.findOne({username: user.username});
                if (isHave.username === user.username && isHave.userId !== user.userId) {
                    return {code: 1, msg: '用户名重复', data: ''};
                }
                await this.userRepository.update(oldUser, user);
                return {msg: '更新成功', data: user};
            }
            return {code: RCode.FAIL, msg: '更新失败', data: ''};
        } catch (e) {
            return {code: RCode.ERROR, msg: '更新失败', data: e};
        }
    }

    async updatePassword(user: User, password: string, oldPassword: string) {
        try {
            const oldUser = await this.userRepository.findOne({userId: user.userId, password: oldPassword});
            console.log(oldUser);
            ...
            return {code: RCode.FAIL, msg: '旧密码不正确', data: ''};
        } catch (e) {
