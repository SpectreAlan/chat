...
export class ChatGateway {
  ...
  @SubscribeMessage('addRemark')
  async addRemark(@ConnectedSocket() client: Socket, @MessageBody() data: UserMap):Promise<any> {
    const isUser = await this.friendRepository.findOne({userId: data.userId, friendId: data.friendId});
    if(isUser) {
      await this.friendRepository.update(isUser, {...isUser, remark: data.remark});
      this.server.to(data.userId).emit('addRemark', { code: RCode.OK, msg: `添加成功`, data });
    } else {
      this.server.to(data.userId).emit('addRemark', {code: RCode.FAIL, msg:'无法修改' });
    }
  }
  ...
  async getAllData(@ConnectedSocket() client: Socket,  @MessageBody() user: User):Promise<any> {
    ...
    if(isUser) {
      ...
      const friendMapData:any = {}
      const friendPromise = friendMap.map(async (item) => {
        friendMapData[item.friendId] = item.remark
      ...
      friends.map((friend, index) => {
        ...
        friend.remark = friendMapData[friend.userId]
      });
      friendArr = friends;
  ...
  @SubscribeMessage('infoGroup')
  async infoGroup(@ConnectedSocket() client: Socket,  @MessageBody() params: GroupMap):Promise<any> {
    const members: GroupMap[] = await this.groupUserRepository.find({groupId: params.groupId});
    console.log(members);
    const group:Group = await this.groupRepository.findOne({groupId: params.groupId})
    const userGather: {[key: string]: User & {groupRole: number}} = {};
    for(let i = 0;i<members.length;i++) {
      const {userId, role} = members[i]
      if(!userGather[userId]) {
        const info:User = await this.userRepository.findOne({userId: userId})
        userGather[userId] = {...info, groupRole: role};
      }
    }
    const list = Object.values(userGather).sort((a,b)=>a.groupRole - b.groupRole)
    this.server.to(params.userId).emit('infoGroup',{code: RCode.OK, msg: '群信息获取成功', data: {...group, members: list}})
  }
