...
export class GroupService {
  ...
  async getGroupInfo(groupId: string) {
    try {
      if(groupId) {
        const info = await this.groupRepository.findOne({groupId: groupId});
        const members = await this.groupUserRepository.find({groupId: groupId});
        return { msg:'获取群的所有用户成功', data: {...info, members}};
      }
      return { msg:'groupId错误', data: null};
    } catch (e) {
      return {code: RCode.ERROR, msg:'获取群信息失败',data: e};
    }
  }
