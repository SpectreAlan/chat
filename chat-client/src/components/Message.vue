  ...
  <div class="message">
    <div class="message-header">
      <div class="message-header-box">
        <a-icon type="left" class="back" @click="back" />
        <span class="message-header-text">{{ chatName }}</span>
        <a-icon type="sync" spin class="message-header-icon" v-if="dropped" />
        <a-icon v-if="groupGather[activeRoom.groupId]" type="team" @click="scanGroup" />
        <active v-else type="friend"></active>
      </div>
    </div>
    ...
    <div class="message-main" :style="{ opacity: messageOpacity }">
      <div class="message-content">
        ...
        <template v-for="item in activeRoom.messages">
          <div class="message-content-message" :key="item.userId + item.time"
               :class="{ 'text-right': item.userId === user.userId }">
            <avatar :data="item"></avatar>
            <div>
              <a class="message-content-text" v-if="_isUrl(item.content)" :href="item.content"
                 target="_blank">{{ item.content }}</a>
              <div class="message-content-text" v-text="_parseText(item.content)"
...
export default class Message extends Vue {
  ...
  back() {
    this.$router.go(-1);
    this._setActiveRoom(null);
  }

  scanGroup() {
    this.$router.push(`/groupDetail?groupId=${this.activeRoom.groupId}`)
  }
  ...
  get chatName() {
    if (this.groupGather[this.activeRoom.groupId]) {
      ...
      return o.remark || o.nickName || o.username;
    }
  }
