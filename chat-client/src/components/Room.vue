    ...
    <div v-for="(chat, index) in chatArr" :key="(chat.userId || chat.groupId) + index">
      ...
      >
        ...
        <div class="room-card-message">
          <div class="room-card-name"><span>{{ chat.groupName }}</span><div class="time">{{formatTime(chat.messages[chat.messages.length - 1].time)}}</div></div>
      ...
      >
        ...
        <div class="room-card-message">
          <div class="room-card-name">
            <span>{{ chat.remark || chat.username }}</span>
            <div class="time">{{formatTime(chat.messages[chat.messages.length - 1].time)}}</div>
          </div>
          <div class="room-card-new" v-if="chat.messages">
            <div
              class="text"
...
export default class Room extends Vue {
  ...
  sortChat() {
    ...
    this.chatArr = [...groups, ...friends].filter(item=>item.messages?.length);
    // 对聊天窗进行排序(根据最新消息时间)
  ...
  changeActiveRoom(activeRoom: User & Group) {
    this._setActiveRoom(activeRoom)
