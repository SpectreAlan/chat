...
import {
  ...
  DEL_GROUP,
  DEL_FRIEND,
  ADD_UNREAD_GATHER, SET_GROUP_INFO
} from "./mutation-types";
...
const actions: ActionTree<ChatState, RootState> = {
  ...
  async connectSocket({ commit, state, dispatch, rootState }, callback) {
    ...
    socket.on("addRemark", (res: ServerRes) => {
      if (!res.code) {
        const user = state.friendGather[res.data.friendId];
        commit(SET_FRIEND_GATHER, { ...user, remark: res.data.remark });
    ...
    socket.on('infoGroup', (res: ServerRes) => {
      if (!res.code) {
        commit(SET_GROUP_INFO, res.data);
      } else {
        Vue.prototype.$message.error(res.msg);
      }
    });
